  ...
  <div class="flex flex-row max-w-md">
    {#each navItems as { href, label, icon } (href)}
      ...
      >
        {#if href === '/typing'}
          <div
            class="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full font-bold shadow-sm z-10"
          >
            NEW
          </div>
        {/if}
